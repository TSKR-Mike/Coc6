        ...
        if levels is None:
            ...
            if mode == 1:
                ...
                backGround = pygwidgets.TextButton(window, (first_x - 50, first_y - 30), '',
                                                   upColor=self.background_color, height=abs(first_y - self.y) + 120,
                                                   width=abs(first_x - self.x) + 300)
            else:
                ...
                backGround = pygwidgets.TextButton(window, (first_x, first_y), '',
                                                   ...
                                                   width=abs(first_x - loc_list_x[-1]))
            ...
            if max(len(i) for i in levels) < 7:
                ...
                cancel = pygwidgets.TextButton(window, (min_x + 80, (max_y - min_y) + 100), 'cancel',
                                               textColor=(0, 0, 0),
                                               upColor=self.button_up, overColor=self.button_over,
                                               downColor=self.button_down)
                done = pygwidgets.TextButton(window, (min_x - 30, (max_y - min_y) + 100), 'done',
                                             textColor=(0, 0, 0),
                                             upColor=self.button_up, overColor=self.button_over,
                                             downColor=self.button_down)
                backGround = pygwidgets.TextButton(window, (min_x - 50, min_y - 30), '',
                                                   upColor=self.background_color,
                                                   height=max_y - min_y + 120,
                                                   width=max_x - min_x + 300)
            elif type(levels[0][4]) == type(levels[0][5]) == type(levels[0][6]) == type(levels[0][7]) == int:
                min_x = min(min([i[0] for i in [r[1] for r in levels]]))
                max_x = max(max([i[0] for i in [r[1] for r in levels]]))
                min_y = min(min([i[1] for i in [r[1] for r in levels]]))
                max_y = max(max([i[0] for i in [r[1] for r in levels]]))
                cancel = pygwidgets.TextButton(window, (min_x + 80, (max_y - min_y) + 100), 'cancel',
                                               textColor=(0, 0, 0),
                                               upColor=self.button_up, overColor=self.button_over,
                                               downColor=self.button_down)
    ...
    while True:
        for event in pygame.event.get():
            ...
            if n.handleEvent(event):
                ...
                m2 = CheckBox(36, ['mean','max','min','standard deviation','mode','variance',
                                   'median','mode','range','percentile','skew','kurtosis','correlation','covariance','cumulative sum',
                                   'cumulative product','cumulative max','cumulative min',
                                   'cumulative mean','cumulative variance','cumulative standard deviation',
                                   ...
                                   'linear-trend', 'trimmed-mean'], 36,
                 window, clock, first_x=40, first_y=10, each_add_x=0, each_add_y=15, auto_adjust=True, buttons_adjust_length=20)
