...
class WindowListViewer:
    ...
    def __init__(self, data:list[list], maxsize:tuple[int,int], window=None, loc:tuple[int, int]=(0, 0)):
        super().__init__()
        self.running = True
        pygame.init()
        data_modified = [['#']]
        ...
        widths = [0 for i in range(len(data[0]) + 1)]
        ...
        if self.width >= maxsize[0]:
            self.widthRatio = self.width / maxsize[0]
